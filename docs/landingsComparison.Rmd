...
We investigate the landings/catch time series for the NEUS shelf. Data sources include:
...
All plots of landings are on the same scale
...
* The SAU estimates total catch approximately 10% higher post 1980 that that of Link. 


```{r linkcatch,echo= F,message=F}
...
# ggplot2::ggplot(data = neus2021) +
#   ggplot2::geom_line(mapping=ggplot2::aes(x=Year,y=Landings/1000))+
#   ggplot2::ylim(0,2600) + 
...
* Mean percentage increase from reported catch to total estimate (SAU data) = `r percentDiscards``
...
```{r comland,echo= F,fig.cap="Reported landings, from comlandr, for the NEUS separated by data source"}
...
* Large discrepancy between `comlandr` reported landings and NAFO reported landings for USA. This is explained below.
...
# nafosplit <- comlandr::get_foreign_data(removeUSA = F,aggregateCountry = F)
# 
# 
# nafo <- nafosplit %>%
#   dplyr::filter(Divcode %in% NEUSDivs) %>%
#   dplyr::group_by(Year) %>%
#   dplyr::summarise(Landings = sum(SPPLIVMT))
# 
# 
# nafosplit <- nafosplit %>% 
#   dplyr::filter(Divcode %in% NEUSDivs) %>% 
...
# 
# nafousa <- nafosplit %>% 
#   dplyr::filter(Country == 22) %>%
#   dplyr::select(-Country) %>% 
#   dplyr::rename(USA = Landings)
# 
# nafoforeign <- nafosplit %>%
#   dplyr::filter(!(Country == 22)) %>%
#   dplyr::group_by(Year) %>%
#   dplyr::summarise(Foreign = sum(Landings),.groups="drop")
# 
# nafoCombined <- nafousa %>%
#   dplyr::full_join(.,nafoforeign,by="Year") %>% 
#   dplyr::full_join(.,comland,by="Year") %>%
#   dplyr::rename(Comland = Landings) %>%
#   tidyr::pivot_longer(.,cols=-Year,names_to = "Source",values_to = "Landings")
# 
#   ggplot2::ggplot(data = nafoCombined) +
#   ggplot2::geom_line(mapping=ggplot2::aes(x=Year,y=Landings/1000,color=Source)) + 
#   ggplot2::ylim(0,2600) +
#   ggplot2::xlim(1950,2020) +
#   ggplot2::ylab("Catch (1000 mt)") +
...
Is this due to Chesapeake bay? The main contributors to the catch are
...
So why are these differences present?
